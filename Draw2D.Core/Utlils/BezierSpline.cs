...
namespace Draw2D.Core.Utlils
{
    ...
    {
        /* Taken from: http://www.codeproject.com/Articles/31859/Draw-a-Smooth-Curve-through-a-Set-of-2D-Points-wit
         * Possible usage:
         * 
         * Point[] cp1; //CP = ControlPoint
                Point[] cp2;

                BezierSpline.GetCurveControlPoints(Points.ToArray(), out cp1,
                   out cp2);

                var bezierSpline = new List<Point>();

                for (int i = 0; i < cp1.Length; i++)
                {
                    bezierSpline.Add(cp1[i]);
                    bezierSpline.Add(cp2[i]);
                    bezierSpline.Add(Points[i + 1]);
                }

                using (StreamGeometryContext ctx = geom.Open())
                {
                    ctx.BeginFigure(toScreenSpace(Points[0]), false, false);

                    ctx.PolyBezierTo(bezierSpline.Select(toScreenSpace).ToList(), true,
                        true);               
                }
         * */
        ...
        private static int[] GetFirstControlPoints(int[] rhs)
        {
            int n = rhs.Length;
            var x = new int[n]; // Solution vector.
            var tmp = new int[n]; // Temp workspace.
